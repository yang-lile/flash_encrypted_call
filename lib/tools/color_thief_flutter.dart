...
// 自定义范围的算法
// 原理：对width+-size/2的范围统计像素
List<List<int>> myCreatePixelArray(
    Uint8List imgData, int quality, int width, int height, int size) {
  final pixels = imgData;
  List<List<int>> pixelArray = [];
  for (var i = (height - size) ~/ 2, offset, r, g, b;
      i < (height + size) ~/ 2;
      i += quality) {
    for (var j = (width - size) ~/ 2; j < (width + size) ~/ 2; j += quality) {
      offset = (i * width + j) * 4;
      r = pixels[offset + 0];
      g = pixels[offset + 1];
      b = pixels[offset + 2];
...
// 自定义了一个对应位置的像素大小的算法
Future myGetPaletteFromImage(Image image, [int colorCount, int quality]) async {
  // 获取要计算的最小的正方形边长，按比例来缩小
  int size = min(image.width, image.height) ~/ 10;
  ...
  final pixelArray =
      myCreatePixelArray(imageData, quality, image.width, image.height, size);
